...
- **`training_bin_stream`** (`neat-core/src/training_bin_stream.rs`) — **one** chunked `.bin` scan API: pipelined double-buffer reads on native hosts, sequential `File::read` chunks on `wasm32` (same `for_each_read_chunk` callback). Used by **NEAT-AI-scorer** for production-sized forward-only scoring. Its second entry point, **`for_each_sampled_read_chunk`** (`training_bin_stream/sampled.rs`, native only), serves a caller that scores a **stratified subsample**: it fetches only the records the caller's `keep(global_record_index)` predicate selects, so a 5 % call stops reading and decoding the 95 % it discards (NEAT-AI-Lamarck#123 measured a **10.7 s → 3.4 s** fixed per-call cost on the 21 GiB production corpus). Sparse reads only beat a sequential sweep with several requests in flight, so it reads over a **pool** — but segment `k` is read by reader `k % readers` and pulled back in `k` order, so the delivered byte stream is identical for every reader count and the caller's float sums stay **bit-identical**. `sampled_read_is_worthwhile` is the gate: ≤ 25 % of records kept and ≥ 64 KiB of mean skip, else keep the full sweep. The stride itself stays with the caller — this module never owns a second copy of it.
