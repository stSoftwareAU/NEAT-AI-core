...
- **violation** — the copy contract is prose with no gate, and no `AGENTS.md` update — evidence: `README.md:198` (the "Canonical `runlib.sh`" section), `RELEASING.md:733` and `AGENTS.md:194` — reason: the `AGENTS.md` half is fixed here — the Repository layout list now carries the `scripts/runlib.sh` entry naming the copy-outward direction and linking the contract rather than restating it. The missing *cross-repo* gate stands: this issue asks for README/RELEASING documentation, and the repository-isolation rule forbids centralising a per-repo gate from another repository, so each sibling's own CI gates its own copy; `tests/scripts/runlib.bats` gates the script itself here
