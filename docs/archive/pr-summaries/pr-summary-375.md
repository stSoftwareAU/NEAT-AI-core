...
| Before | After |
| --- | --- |
| "The PRODUCTION selection lives in" the named private repository, with its `owner/repo` slug | "The PRODUCTION selection lives in the downstream production training system, which this public repository does not contain" |
...
| "so node.sh cannot downgrade a marker-less OOM abort to success", citing a private issue slug alongside Issue #3234 | "so the parent launcher cannot downgrade a marker-less OOM abort to success" |
| "CI-guarded in" the named private repository "by `test/worker/MemoryCalcHeapSize.ts`" | "The production selector has its own CI gates in that downstream system" |
| "lock-step with `memory_calc.sh` … divergence …" and the named private repository's | "lock-step with the production memory-sizing formula … and the production selector's" |
| "the … tier" / "…'s signature", both naming a private issue slug | "the OOME tier" / "the OOME's signature" |
| a private host identifier: "8 GB total but only 3865 MB available" | "The constrained-host case: 8 GB total but only 3865 MB available" |
| the two private repo-prefixed environment variables | "the max-heap cap" / "opt-in and off for the learn path" |
...
Also repointed the README lane (d) link from the old lane (d) doc path — which
still carried the private repository's name in its filename — to the renamed
...
```mermaid
flowchart LR
    A["private names:<br/>downstream trainer repo<br/>and its owner/repo slug,<br/>two private issue slugs,<br/>memory_calc.sh, node.sh"] --> B["concept-level reword<br/>(comments only)"]
    B --> C["downstream production<br/>training system /<br/>learn launcher /<br/>production selector /<br/>marker-less non-zero exit"]
    D["README lane (d) link<br/>(old private-named path)"] --> E["renamed doc path<br/>— link resolves again"]
```
...
- A word-boundary grep of `tests/perf/` for the private repository's name — no
  matches.
...
- none names the private repository (its name as a word-boundary-matched token);
- none links a private repository path or issue slug;
