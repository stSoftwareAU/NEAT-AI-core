...
| `deny.toml` | `cargo deny` (licences, advisories, bans). The `[bans]` wrappers bound the unmaintained `winapi ← page_size ← criterion`, `tinytemplate ← criterion` and `alloca ← criterion` chains to their one dev-dependency path — see [Orphaned transitive crates](SECURITY.md#orphaned-transitive-crates-issues-676-677-720); `tests/cargo_orphan_containment_test.ts` is the gate that fails if the policy or the chain changes. |
...
| `scripts/lockfile-freshness.sh` | Lockfile-freshness gate and re-lock (Issue #695). `--check` fails when any committed `Cargo.lock` no longer resolves against the manifest beside it; `--update` re-locks them, which is what the CI `version-increment` job runs after its version bump so the lockfiles outside the root workspace stop naming a superseded `neat-core`. Both modes sweep every `Cargo.lock` in the tree, so a third lockfile is covered without being wired in by hand. Gated by `tests/scripts/lockfile_freshness.bats`. |
