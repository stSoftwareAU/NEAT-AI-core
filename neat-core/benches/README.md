...
| `weighted_sum_simd` | `weighted_sum_simd` family (single / no-bias / squares / 4- and 8-record). Measures the `*_unchecked` kernels, which are what the forward pass runs once `CompiledNetwork::new` has discharged the index precondition (Issue #613); `single_checked` measures the safe entry point beside them, so the `O(end - start)` bounds pre-pass a caller holding no loaded network pays is reproducible from the committed tree | 64-synapse block |
