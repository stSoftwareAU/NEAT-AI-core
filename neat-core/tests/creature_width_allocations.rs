...
//! width and were the entry points Issue #622 left out (Issue #639). They owe
//! NEAT-AI's own rule wording rather than a typed `CreatureError`, so they
//! carry the ceiling as a rule of their own; the cost of the refusal is
//! measured here exactly as it is for the two above.
//!
...
use neat_core::{
    CreatureError, CreatureExport, NeuronExport, SynapseExport, ValidateOptions, ValidationStats,
...
fn creature_validate_refuses_an_oversized_declared_input_without_paying_for_it() {
    ...
    assert_width_is_not_walked("creature_validate", |creature| {
        creature_validate(creature, &ValidateOptions::default())
            .is_err_and(|failure| failure.message.contains("exceeding the maximum"))
    });
}
